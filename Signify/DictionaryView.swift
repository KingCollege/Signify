        ...
        NavigationView {
            ZStack(){
                ScrollView(showsIndicators:false){
                    ...
                    VStack(alignment:.leading){
                        ForEach(0..<alphabet.count, id: \.self) { index in
                            HStack(spacing:20) { ForEach(self.alphabet[index].filter{$0.hasPrefix(self.text)}, id: \.self) { letter in
                ...
                .navigationBarItems(leading: HStack(){
                    Text(screenTitle).font(.largeTitle).fontWeight(.bold)
                }.padding(.top)
                    , trailing :             NavigationLink(destination: QuizView()){
